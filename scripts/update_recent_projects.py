...
def latest_commit(repo):
    ...
    if not date:
        return None
    return date, c.get("html_url", repo["html_url"])
...
for repo in repos:
    ...
    if latest:
        items.append((latest[0], latest[1], repo))

items.sort(key=lambda x: x[0], reverse=True)
lines = []
for date, commit_url, repo in items[:LIMIT]:
    day = datetime.fromisoformat(date.replace("Z", "+00:00")).strftime("%Y-%m-%d")
    desc = (repo.get("description") or "").strip()
    lang = repo.get("language") or ""
    meta = " · ".join(x for x in [lang, f"updated {day}"] if x)
    lines.append(f"### [{repo['name']}]({repo['html_url']})")
    if desc:
        lines.append(desc)
    lines.append(f"<sub>{meta} · [latest commit]({commit_url})</sub>\n")

block = START + "\n" + ("\n".join(lines) if lines else "_No public projects yet._") + "\n" + END
